...
public class ClassTeachingAssignmentBusinessRules(IPlanoraUnitOfWork planoraUnitOfWork):BaseBusinessRules
{
    public async Task<ClassTeachingAssignment> EnrichClassTeachingAssignmentAsync(ClassTeachingAssignment entity, CancellationToken cancellationToken)
    {
        ...
        var classSection = await planoraUnitOfWork.ClassSections.GetAsync(c => c.FakeId == entity.ClassSectionFakeId, cancellationToken: cancellationToken);
        var teacher = await planoraUnitOfWork.Teachers.GetAsync(t => t.FakeId == entity.TeacherFakeId, cancellationToken: cancellationToken);
        var lecture = await planoraUnitOfWork.Lectures.GetAsync(l => l.FakeId == entity.LectureFakeId, cancellationToken: cancellationToken);
        ...
        entity.ClassSectionId = classSection.Id;
        entity.TeacherId = teacher.Id;
        entity.LectureId = lecture.Id;
