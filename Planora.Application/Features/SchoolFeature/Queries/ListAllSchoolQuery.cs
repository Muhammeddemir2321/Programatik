...
public class ListAllSchoolQuery : IRequest<SchoolListModel>, ISecuredRequest
{
            ...
            : IRequestHandler<ListAllSchoolQuery, SchoolListModel>
    {
        ...
        {
            var schools = await schoolRepository.GetListAsync(index: request.PageRequest.Page, size: request.PageRequest.PageSize, cancellationToken: cancellationToken);
            return mapper.Map<SchoolListModel>(schools);
        }
    }
}
